...
class Profesor {
    ...
    public function autenticar($usuario, $clave) {
        ...
        $consulta = "SELECT * FROM profesores WHERE usuario='$usuario' AND contra='$clave'";
    ...
    {
        $sql = "SELECT MAX(id) AS ultimo_id FROM profesores";
 ...
 /**
     ...
     */
    public function crearProfesor($id, $nombre, $apellido, $email, $usuario, $clave, $cedula, $telefono) {
        // Llamar al procedimiento almacenado en la base de datos
        $query = "BEGIN crear_profesor_proc(:id, :nombre, :apellido, :email, :usuario, :clave, :cedula, :telefono); END;";
